...
const DemoBanner = () => {
  ...
  return <div className="fixed top-0 left-0 right-0 z-[60] bg-gradient-to-r from-primary/90 via-primary to-primary/90 backdrop-blur-sm border-b border-primary-foreground/10">
      <div className="container py-2.5 flex items-center gap-4">
        ...
        <button onClick={() => setDismissed(true)} className="text-primary-foreground/60 hover:text-primary-foreground transition-colors p-1 ml-auto" aria-label="Dismiss banner">
          <X className="w-4 h-4" />
        </button>
      </div>
    </div>;
